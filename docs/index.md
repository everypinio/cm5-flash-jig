...
| Raspberry Pi Compute Module 4 with eMMC | Validation pending | The available CM4 test points do not appear to expose the 5 V power, USB, UART, or boot signals required by the jig, so flashing through these points is unlikely to be possible. [Open the interactive CM4 test-point map](test-points/cm4/). |
